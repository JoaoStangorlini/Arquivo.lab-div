-- ==========================================================
-- 🛡️ HUB DE COMUNICAÇÃO CIENTÍFICA - GOLDEN MASTER V3.9
-- Migração: v26_final_trigger.sql (REVISED)
-- Objetivo: Proteção Atômica contra o "Status Shift Bug"
-- Permite edições (Self-Edit) e novos posts sem conflito.
-- ==========================================================
...
BEGIN
    ...
    IF (NEW.use_pseudonym = true AND NEW.status IN ('pendente', 'aprovado')) THEN
        ...
        -- Contagem Atômica V3.9:
        -- 1. Pertence ao mesmo usuário
        -- 2. Usa pseudônimo
        -- 3. Está ativa (pendente/aprovado)
        -- 4. NÃO é o registro atual (Garante resiliência em UPDATE e INSERT)
        SELECT count(id) INTO v_count 
        FROM public.submissions 
        WHERE user_id = NEW.user_id 
          AND use_pseudonym = true 
          AND status IN ('pendente', 'aprovado')
          AND (NEW.id IS NULL OR id <> NEW.id); -- 🛡️ ATOMIC INSERT/UPDATE GUARD
...
-- Re-vincula o trigger (Garante aplicação da lógica V3.9)
...
COMMENT ON FUNCTION public.check_pseudonym_limit() IS 'Enforces a strict limit of 2 active pseudonyms per account, with self-edit resilience (V3.9).';
